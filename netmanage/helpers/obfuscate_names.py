...
This module provides functionality to obfuscate and reverse data in SQLite databases.
It uses a pre-defined mapping of columns to apply specific transformations based on the column's semantic role.
The module uses random character mapping to obfuscate string data.
This ensures referential integrity while making it visually obscured for demonstrations.
...
Functions:
    ...
    - copy_table_structure: Copies the schema of a table from one SQLite database to another.
    - reverse_consistent_transform: Reverse the obfuscation based on predefined rules.
...
# Keys are column names and values are dictionaries that specify 'ignores' and 'prefix'
columns_to_rename = {
...
def consistent_transform(s: str, ignores: list = [], prefix: str = '') -> str:
    ...
    Args:
        ...
        ignores (list, optional): List of values to ignore while transforming. Defaults to [].
        prefix (str, optional): Prefix to add to the transformed string. Defaults to ''.
    ...
    return prefix + ''.join(transformed_words) if prefix else ''.join(transformed_words)


def copy_table_structure(source_cursor, dest_cursor, dest_conn, table_name):
    """Copy the table structure from a source database to a destination database.
...
def reverse_consistent_transform(s: str, reverse_map, ignores: list = [], prefix: str = '') -> str:
    """Reverse the transformation applied to a string back to its original value based on provided rules.

    Args:
        s (str): The transformed string.
        reverse_map (dict): The reverse mapping dictionary.
        ignores (list, optional): List of values to ignore while transforming. Defaults to [].
        prefix (str, optional): Prefix to remove from the transformed string. Defaults to ''.
...
def obfuscate_db_data(source_db_path: str, dest_db_path: str):
    """Obfuscate the database data from a source database to a destination database.
    ...
    dest_cursor.execute("CREATE TABLE IF NOT EXISTS char_map (original TEXT, mapped TEXT);")
    for original, mapped in FORWARD_MAP.items():
        dest_cursor.execute("INSERT INTO char_map (original, mapped) VALUES (?, ?);", (original, mapped))
    dest_conn.commit()
...
def reverse_db_data(source_db_path: str, dest_db_path: str):
    """Reverse the obfuscation of database data from a source to a destination database.
    ...
    for table in tables:
        ...
        for row in rows:
            ...
            for col, original_value in zip(columns, row):
                ...
                rules = columns_to_rename.get(col, {})
                if col in columns_to_rename:
                    new_value = reverse_consistent_transform(original_value, reverse_map, rules.get("ignores", []),
                                                             prefix=rules.get("prefix", ""))
                else:
                    new_value = original_value
                new_values.append(new_value)
...
def test_random_map_transform_and_reverse():
    ...
    print(f"Testing random_map_transform and reverse_map_transform with input: '{test_str}'")
    ...
    assert reversed_str == test_str, "Test for random_map_transform and reverse_map_transform failed."


def test_consistent_transform_and_reverse():
    test_str = "msp-core-switch-01"
    rules = {'ignores': ['switch'], 'prefix': 'test-'}
    print(f"Testing consistent_transform and reverse_consistent_transform with input: '{test_str}'")

    transformed_str = consistent_transform(test_str, **rules)
    print(f"Transformed string: '{transformed_str}'")

    reversed_str = reverse_consistent_transform(transformed_str, REVERSE_MAP, **rules)
    print(f"Reversed string: '{reversed_str}'")

    assert reversed_str == test_str, "Test for consistent_transform and reverse_consistent_transform failed."
