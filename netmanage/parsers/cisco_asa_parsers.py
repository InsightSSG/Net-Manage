...
def asa_parse_interface_ips(results: dict) -> pd.DataFrame:
    """
    ...
    ----------
    results : dict
        A dictionary containing results for 'get_interface_ip_addresses'.
    ...
    for key, value in results.items():
        device = key
        output = value['Interface IP Addresses']['get_interface_ip_addresses']
        output = output.split('\n')
...
def asa_parse_inventory(results: dict) -> pd.DataFrame:
    """
    ...
    ----------
    results : dict
        A dictionary containing results for 'get_inventory'.
    ...
    for key, value in results.items():
        device = key
        data = value['Inventory']['get_inventory'].split('\n')
